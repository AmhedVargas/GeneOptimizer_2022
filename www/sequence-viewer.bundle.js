/*!
 * jQuery2 JavaScript Library v2.1.4
 * http://jQuery2.com/
 *
 * Includes Sizzle.js
 * http://sizzlejs.com/
 *
 * Copyright 2005, 2014 jQuery2 Foundation, Inc. and other contributors
 * Released under the MIT license
 * http://jQuery2.org/license
 *
...
(function( global, factory ) {
	...
	if ( typeof module === "object" && typeof module.exports === "object" ) {
		...
		// is present, execute the factory and get jQuery2.
		// For environments that do not have a `window` with a `document`
		// (such as Node.js), expose a factory as module.exports.
		// This accentuates the need for the creation of a real `window`.
		// e.g. var jQuery2 = require("jQuery2")(window);
		// See ticket #14549 for more info.
		module.exports = global.document ?
			factory( global, true ) :
			function( w ) {
				if ( !w.document ) {
					throw new Error( "jQuery2 requires a window with a document" );
				}
				return factory( w );
			};
	} else {
...
var
	...
	// Define a local copy of jQuery2
	jQuery2 = function( selector, context ) {
		// The jQuery2 object is actually just the init constructor 'enhanced'
		// Need init if jQuery2 is called (just allow error to be thrown if not included)
	...
	// Used by jQuery2.camelCase as callback to replace()
	fcamelCase = function( all, letter ) {
		return letter.toUpperCase();
	};

jQuery2.fn = jQuery2.prototype = {
	// The current version of jQuery2 being used
	jQuery2: version,
	...
	// The default length of a jQuery2 object is 0
	length: 0,
	...
	pushStack: function( elems ) {
		...
		// Build a new jQuery2 matched element set
	...
	// Behaves like an Array's method, not like a jQuery2 method.
	push: push,
	sort: arr.sort,
	splice: arr.splice
};
...
jQuery2.extend = jQuery2.fn.extend = function() {
	...
	// Extend jQuery2 itself if only one argument is passed
	if ( i === length ) {
		target = this;
		i--;
	}
...
jQuery2.extend({
	// Unique for each copy of jQuery2 on the page
	expando: "jQuery2" + ( version + Math.random() ).replace( /\D/g, "" ),

	// Assume jQuery2 is ready without the ready module
	isReady: true,
	...
	// jQuery2.support is not used in Core but other projects attach their
 ...
 * http://sizzlejs.com/
 *
 * Copyright 2008, 2014 jQuery2 Foundation, Inc. and other contributors
 * Released under the MIT license
 * http://jQuery2.org/license
 *
 * Date: 2014-12-16
		...
		if ( nodeType !== 11 && (match = rquickExpr.exec( selector )) ) {
			...
			if ( (m = match[1]) ) {
				if ( nodeType === 9 ) {
					...
					// nodes that are no longer in the document (jQuery2 #6963)
...
setDocument = Sizzle.setDocument = function( node ) {
	...
	// IE will throw "permission denied" error when accessing "document" variable, see jQuery2 #13936
	...
	// See http://bugs.jQuery2.com/ticket/13378
	rbuggyQSA = [];
	...
	if ( (support.qsa = rnative.test( doc.querySelectorAll )) ) {
		...
		assert(function( div ) {
			...
			// http://bugs.jQuery2.com/ticket/12359
...
Sizzle.attr = function( elem, name ) {
	...
	var fn = Expr.attrHandle[ name.toLowerCase() ],
		// Don't get fooled by Object.prototype properties (jQuery2 #13807)
...
Sizzle.uniqueSort = function( results ) {
	...
	// See https://github.com/jQuery2/sizzle/pull/225
	sortInput = null;
...
getText = Sizzle.getText = function( elem ) {
	...
	if ( !nodeType ) {
		...
		// innerText usage removed for consistency of new lines (jQuery2 #11153)
...
jQuery2.filter = function( expr, elems, not ) {
	...
	return elems.length === 1 && elem.nodeType === 1 ?
		jQuery2.find.matchesSelector( elem, expr ) ? [ elem ] : [] :
		jQuery2.find.matches( expr, jQuery2.grep( elems, function( elem ) {
...
// Initialize a jQuery2 object


// A central reference to the root jQuery2(document)
var rootjQuery2,
	...
	init = jQuery2.fn.init = function( selector, context ) {
		...
		if ( typeof selector === "string" ) {
			...
			if ( match && (match[1] || !context) ) {
				...
				// HANDLE: $(#id)
					...
					if ( elem && elem.parentNode ) {
						// Inject the element directly into the jQuery2 object
						this.length = 1;
						this[0] = elem;
					}
			...
			// HANDLE: $(expr, $(...))
			} else if ( !context || context.jQuery2 ) {
		...
		// Shortcut for document ready
			...
			return typeof rootjQuery2.ready !== "undefined" ?
				rootjQuery2.ready( selector ) :
...
// Give the init function the jQuery2 prototype for later instantiation
init.prototype = jQuery2.fn;

// Initialize central reference
rootjQuery2 = jQuery2( document );
...
jQuery2.fn.extend({
	...
	closest: function( selectors, context ) {
		var cur,
			...
			pos = rneedsContext.test( selectors ) || typeof selectors !== "string" ?
				jQuery2( selectors, context || this.context ) :
				0;
	...
	index: function( elem ) {
		...
		return indexOf.call( this,
			...
			// If it receives a jQuery2 object, the first element is used
			elem.jQuery2 ? elem[ 0 ] : elem
		);
	},
...
jQuery2.extend({
	...
	when: function( subordinate /* , ..., subordinateN */ ) {
		var i = 0,
			...
			remaining = length !== 1 || ( subordinate && jQuery2.isFunction( subordinate.promise ) ) ? length : 0,

			// the master Deferred. If resolveValues consist of only a single Deferred, just use that.
			deferred = remaining === 1 ? subordinate : jQuery2.Deferred(),
...
jQuery2.ready.promise = function( obj ) {
	if ( !readyList ) {
		...
		// discovered by ChrisS here: http://bugs.jQuery2.com/ticket/12282#comment:15
		...
		if ( bulk ) {
			...
			// ...except when executing function values
				...
				fn = function( elem, key, value ) {
					return bulk.call( jQuery2( elem ), value );
				};
			}
		}
	...
	access: function( owner, key, value ) {
		...
		if ( key === undefined ||
			...
			return stored !== undefined ?
				stored : this.get( owner, jQuery2.camelCase(key) );
		}
		...
		if ( typeof data === "string" ) {
			try {
				data = data === "true" ? true :
					...
					+data + "" === data ? +data :
					rbrace.test( data ) ? jQuery2.parseJSON( data ) :
					data;
			} catch( e ) {}
...
jQuery2.fn.extend({
	data: function( key, value ) {
		...
		return access( this, function( value ) {
			...
			// The calling jQuery2 object (element matches) is not empty
			// (and therefore has an element appears at this[ 0 ]) and the
			// `value` parameter was not undefined. An empty jQuery2 object
...
var isHidden = function( elem, el ) {
		// isHidden might be called from jQuery2#filter function;
...
jQuery2.event = {
	...
	add: function( elem, types, handler, data, selector ) {
		...
		if ( !(eventHandle = elemData.handle) ) {
			eventHandle = elemData.handle = function( e ) {
				// Discard the second event of a jQuery2.event.trigger() and
				// when an event is called after a page has unloaded
				return typeof jQuery2 !== strundefined && jQuery2.event.triggered !== e.type ?
					jQuery2.event.dispatch.apply( elem, arguments ) : undefined;
			};
		}
	...
	trigger: function( event, data, elem, onlyHandlers ) {
		...
		// Caller can pass in a jQuery2.Event object, Object, or just an event type string
		event = event[ jQuery2.expando ] ?
			event :
			new jQuery2.Event( type, typeof event === "object" && event );

		// Trigger bitmask: & 1 for native handlers; & 2 for jQuery2 (always true)
	...
	dispatch: function( event ) {
		...
		// Make a writable jQuery2.Event from the native event object
		...
		// Use the fix-ed jQuery2.Event rather than the (read-only) native event
		args[0] = event;
		...
		while ( (matched = handlerQueue[ i++ ]) && !event.isPropagationStopped() ) {
			...
			while ( (handleObj = matched.handlers[ j++ ]) && !event.isImmediatePropagationStopped() ) {
				...
				if ( !event.namespace_re || event.namespace_re.test( handleObj.namespace ) ) {
					...
					ret = ( (jQuery2.event.special[ handleObj.origType ] || {}).handle || handleObj.handler )
...
// jQuery2.Event is based on DOM3 Events as specified by the ECMAScript Language Binding
...
jQuery2.fn.extend({
	...
	off: function( types, selector, fn ) {
		...
		if ( types && types.preventDefault && types.handleObj ) {
			// ( event )  dispatched jQuery2.Event
...
function manipulationTarget( elem, content ) {
	return jQuery2.nodeName( elem, "table" ) &&
		jQuery2.nodeName( content.nodeType !== 11 ? content : content.firstChild, "tr" ) ?
...
function getAll( context, tag ) {
	...
	return tag === undefined || tag && jQuery2.nodeName( context, tag ) ?
		jQuery2.merge( [ context ], ret ) :
		ret;
}
...
jQuery2.extend({
	clone: function( elem, dataAndEvents, deepDataAndEvents ) {
		var i, l, srcElements, destElements,
			...
			inPage = jQuery2.contains( elem.ownerDocument, elem );
				...
				if ( key && (data = data_priv.cache[ key ]) ) {
					if ( data.events ) {
						for ( type in data.events ) {
							...
							// This is a shortcut to avoid jQuery2.event.remove's overhead
							} else {
...
jQuery2.fn.extend({
	...
	remove: function( selector, keepData /* Internal Use Only */ ) {
		var elem,
			elems = selector ? jQuery2.filter( selector, this ) : this,
			i = 0;
			...
			if ( first ) {
				...
				for ( ; i < l; i++ ) {
					...
					if ( i !== iNoClone ) {
						...
						if ( hasScripts ) {
							// Support: QtWebKit
							// jQuery2.merge because push.apply(_, arraylike) throws
				...
				if ( hasScripts ) {
					...
					for ( i = 0; i < hasScripts; i++ ) {
						...
						if ( rscriptType.test( node.type || "" ) &&
							!data_priv.access( node, "globalEval" ) && jQuery2.contains( doc, node ) ) {
...
function actualDisplay( name, doc ) {
	var style,
		elem = jQuery2( doc.createElement( name ) ).appendTo( doc.body ),
		...
		display = window.getDefaultComputedStyle && ( style = window.getDefaultComputedStyle( elem[ 0 ] ) ) ?
			...
			style.display : jQuery2.css( elem[ 0 ], "display" );
...
jQuery2.extend({
	...
	style: function( elem, name, value, extra ) {
		...
		var ret, type, hooks,
			origName = jQuery2.camelCase( name ),
			style = elem.style;
	...
	jQuery2.cssHooks[ name ] = {
		get: function( elem, computed, extra ) {
			if ( computed ) {
				...
				return rdisplayswap.test( jQuery2.css( elem, "display" ) ) && elem.offsetWidth === 0 ?
					jQuery2.swap( elem, cssShow, function() {
...
// Support: Android 2.3
jQuery2.cssHooks.marginRight = addGetHookIf( support.reliableMarginRight,
	function( elem, computed ) {
		if ( computed ) {
			return jQuery2.swap( elem, { "display": "inline-block" },
	...
	css: function( name, value ) {
		return access( this, function( elem, name, value ) {
			...
			if ( jQuery2.isArray( name ) ) {
				...
				for ( ; i < len; i++ ) {
					map[ name[ i ] ] = jQuery2.css( elem, name[ i ], false, styles );
				}
			...
			return value !== undefined ?
				jQuery2.style( elem, name, value ) :
				jQuery2.css( elem, name );
	...
	tweeners = {
		"*": [ function( prop, value ) {
			var tween = this.createTween( prop, value ),
				...
				unit = parts && parts[ 3 ] || ( jQuery2.cssNumber[ prop ] ? "" : "px" ),

				// Starting value computation is required for potential unit mismatches
				start = ( jQuery2.cssNumber[ prop ] || unit !== "px" && +target ) &&
					rfxnum.exec( jQuery2.css( tween.elem, prop ) ),
				scale = 1,
				maxIterations = 20;

			if ( start && start[ 3 ] !== unit ) {
				// Trust units reported by jQuery2.css
				unit = unit || start[ 3 ];
	...
	var result,
		...
		animation = deferred.promise({
			...
			createTween: function( prop, end ) {
				var tween = jQuery2.Tween( elem, animation.opts, prop, end,
...
jQuery2.speed = function( speed, easing, fn ) {
	var opt = speed && typeof speed === "object" ? jQuery2.extend( {}, speed ) : {
		complete: fn || !fn && easing ||
			jQuery2.isFunction( speed ) && speed,
		duration: speed,
		easing: fn && easing || easing && !jQuery2.isFunction( easing ) && easing
	};

	opt.duration = jQuery2.fx.off ? 0 : typeof opt.duration === "number" ? opt.duration :
		opt.duration in jQuery2.fx.speeds ? jQuery2.fx.speeds[ opt.duration ] : jQuery2.fx.speeds._default;
...
// http://blindsignals.com/index.php/2009/07/jQuery2-delay/
...
var nodeHook, boolHook,
	attrHandle = jQuery2.expr.attrHandle;
...
jQuery2.extend({
	attr: function( elem, name, value ) {
		...
		if ( nType !== 1 || !jQuery2.isXMLDoc( elem ) ) {
			...
			hooks = jQuery2.attrHooks[ name ] ||
				( jQuery2.expr.match.bool.test( name ) ? boolHook : nodeHook );
		}
...
jQuery2.fn.extend({
	addClass: function( value ) {
		...
		if ( jQuery2.isFunction( value ) ) {
			return this.each(function( j ) {
				jQuery2( this ).addClass( value.call( this, j, this.className ) );
			});
		}
	...
	removeClass: function( value ) {
		...
		if ( jQuery2.isFunction( value ) ) {
			return this.each(function( j ) {
				jQuery2( this ).removeClass( value.call( this, j, this.className ) );
			});
		}
		if ( proceed ) {
			classes = ( value || "" ).match( rnotwhite ) || [];
	...
	val: function( value ) {
		...
		return this.each(function( i ) {
			...
			if ( val == null ) {
				...
				val = jQuery2.map( val, function( value ) {
		...
		option: {
			get: function( elem ) {
				...
				return val != null ?
					...
					jQuery2.trim( jQuery2.text( elem ) );
			}
		},
		select: {
			get: function( elem ) {
				...
				for ( ; i < max; i++ ) {
					...
					if ( ( option.selected || i === index ) &&
							...
							( !option.parentNode.disabled || !jQuery2.nodeName( option.parentNode, "optgroup" ) ) ) {
...
// Return jQuery2 for attributes-only inclusion


jQuery2.each( ("blur focus focusin focusout load resize scroll unload click dblclick " +
...
// Base "constructor" for jQuery2.ajaxPrefilter and jQuery2.ajaxTransport
...
jQuery2.extend({
	...
	ajax: function( url, options ) {
		...
		var transport,
			...
			// Context for global events is callbackContext if it is a DOM node or jQuery2 collection
			globalEventContext = s.context && ( callbackContext.nodeType || callbackContext.jQuery2 ) ?
		...
		// Don't fire events if jQuery2.event is undefined in an AMD-usage scenario (#15118)
...
jQuery2.param = function( a, traditional ) {
	...
	// Set traditional to true for jQuery2 <= 1.3.2 behavior.
	if ( traditional === undefined ) {
		traditional = jQuery2.ajaxSettings && jQuery2.ajaxSettings.traditional;
	}

	// If an array was passed in, assume that it is an array of form elements.
	if ( jQuery2.isArray( a ) || ( a.jQuery2 && !jQuery2.isPlainObject( a ) ) ) {
...
jQuery2.fn.extend({
	...
	serializeArray: function() {
		...
		.filter(function() {
			...
			return this.name && !jQuery2( this ).is( ":disabled" ) &&
...
jQuery2.ajaxPrefilter( "json jsonp", function( s, originalSettings, jqXHR ) {
	...
	if ( jsonProp || s.dataTypes[ 0 ] === "jsonp" ) {
		...
		callbackName = s.jsonpCallback = jQuery2.isFunction( s.jsonpCallback ) ?
			s.jsonpCallback() :
			s.jsonpCallback;
...
// Add the top/left cssHooks using jQuery2.fn.position
// Webkit bug: https://bugs.webkit.org/show_bug.cgi?id=29084
// Blink bug: https://code.google.com/p/chromium/issues/detail?id=229280
// getComputedStyle returns percent when specified for top/left/bottom/right;
// rather than make the css module depend on the offset module, just check for it here
jQuery2.each( [ "top", "left" ], function( i, prop ) {
	jQuery2.cssHooks[ prop ] = addGetHookIf( support.pixelPosition,
		function( elem, computed ) {
			if ( computed ) {
				computed = curCSS( elem, prop );
				// If curCSS returns percentage, fallback to offset
				return rnumnonpx.test( computed ) ?
					jQuery2( elem ).position()[ prop ] + "px" :
					computed;
			}
		}
	);
		...
		jQuery2.fn[ funcName ] = function( margin, value ) {
			...
			return access( this, function( elem, type, value ) {
				...
				if ( jQuery2.isWindow( elem ) ) {
					...
					// https://github.com/jQuery2/jQuery2/pull/764
				...
				return value === undefined ?
					...
					jQuery2.css( elem, type, extra ) :

					// Set width or height on the element
					jQuery2.style( elem, type, value, extra );
...
// Register as a named AMD module, since jQuery2 can be concatenated with other
// files that may use define, but not via a proper concatenation script that
// understands anonymous AMD modules. A named AMD is safest and most robust
// way to register. Lowercase jQuery2 is used because AMD module names are
// derived from file names, and jQuery2 is normally delivered in a lowercase
// file name. Do this after creating the global so that if an AMD module wants
// to call noConflict to hide this version of jQuery2, it will work.

// Note that for maximum portability, libraries that are not jQuery2 should
// declare themselves as anonymous modules, and avoid setting a global if an
// AMD loader is present. jQuery2 is a special case. For more information, see
...
var
	// Map over jQuery2 in case of overwrite
	_jQuery2 = window.jQuery2,
...
jQuery2.noConflict = function( deep ) {
	...
	if ( deep && window.jQuery2 === jQuery2 ) {
		window.jQuery2 = _jQuery2;
	}

	return jQuery2;
};

// Expose jQuery2 and $ identifiers, even in AMD
// (#7102#comment:10, https://github.com/jQuery2/jQuery2/pull/557)
// and CommonJS for browser emulators (#13566)
if ( typeof noGlobal === strundefined ) {
	window.jQuery2 = window.$ = jQuery2;
}
...
//            else sequenceOptions = options;
    ...
    //                        '</div>'
        ...
        this.coverage = function (HashAA, start, end, highlightColor) {
            seqCoverage.data = jQuery2.extend(true, [], HashAA);
